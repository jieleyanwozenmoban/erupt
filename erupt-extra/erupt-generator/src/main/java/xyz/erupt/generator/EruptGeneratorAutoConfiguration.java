...
public class EruptGeneratorAutoConfiguration implements EruptModule {
    ...
    public ModuleInfo info() {
        return ModuleInfo.builder().name("erupt-generator").description("Generator erupt code").build();
    }

    @Override
    public List<MetaMenu> initMenus() {
        List<MetaMenu> menus = new ArrayList<>();
        menus.add(MetaMenu.createRootMenu("$generator", "Generate Code", "fa fa-code", 40));
